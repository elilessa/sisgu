...
export function AgendaComercial() {
  ...
  const handleCriarOrcamento = async () => {
    ...
    try {
      ...
      // Lógica robusta de geração de número (Padrão ORÇ-AAMMNNNNN)
      const now = new Date();
      ...
      const numeroOrcamento = `${prefixo}${proximoNumero.toString().padStart(5, '0')}`;
        ...
        <>
          ...
          {filteredChamados.map((chamado) => (
            <Accordion
              ...
              sx={{
                mb: 1,
                bgcolor: (chamado.status === 'pendencia_financeira'
                  && chamado.pendenciaFinanceira?.tipo === 'orcamento'
            ...
            >
              <AccordionSummary expandIcon={<ExpandMore />}>
                <Box className="flex items-center gap-2 w-full">
                  ...
                  {/* Não mostrar status 'Pendência Financeira' se for orçamento direto da agenda técnica (para não mostrar laranja) */}
                  {!(chamado.status === 'pendencia_financeira' &&
                    chamado.pendenciaFinanceira?.tipo === 'orcamento' &&
                    (!chamado.agendamentos || chamado.agendamentos.length === 0)) && (
                      <StatusChip
