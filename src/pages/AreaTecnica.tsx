...
export function AreaTecnica() {
  ...
  const loadClientes = async () => {
    if (!userData?.empresaId) return;
    try {
      const ref = collection(db, 'EMPRESAS', userData.empresaId, 'clientes');
      const snap = await getDocs(ref);
      const list: Cliente[] = [];
      snap.forEach((d) => list.push({ id: d.id, prefixoNome: d.data().prefixoNome }));
      setClientes(list);
    } catch (err) {
  ...
  const gerarTextoRelatorio = (agendamento: Agendamento): string => {
    ...
    texto += `Tipo de Finalização: ${agendamento.tipoFinalizacao === 'sem_pendencia' ? 'Sem pendência' :
      agendamento.tipoFinalizacao === 'pendencia_tecnica' ? 'Com pendência t�cnica' :
        agendamento.tipoFinalizacao === 'pendencia_financeira' ? 'Com pendência financeira' : 'Nao informado'
      }\n`;
  ...
  const handleGerarPDF = (chamado: Chamado) => {
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.text('DESCRIÇÃO DO CHAMADO', margin, yPos);
    yPos += lineHeight;
    pdf.setFont('helvetica', 'normal');
    const descLines = pdf.splitTextToSize(chamado.descricao || 'Sem descrição', pageWidth - margin * 2);
    ...
    if (chamado.pendenciaTecnica) {
      ...
      pdf.text('PENDÊNCIA TÉCNICA', margin, yPos);
      yPos += lineHeight;
      pdf.setFont('helvetica', 'normal');
    ...
    if (chamado.pendenciaFinanceira) {
      ...
      pdf.text(`PENDÊNCIA FINANCEIRA (${chamado.pendenciaFinanceira.tipo === 'cobranca' ? 'Cobrança' : 'Orçamento'})`, margin, yPos);
      yPos += lineHeight;
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(0, 0, 0);
    ...
    if (chamado.agendamentos && chamado.agendamentos.length > 0) {
      pdf.setFont('helvetica', 'bold');
      pdf.text('HISTÓRICO DE AGENDAMENTOS', margin, yPos);
      yPos += lineHeight * 1.5;
      ...
      chamado.agendamentos.forEach((ag, idx) => {
        ...
        yPos += lineHeight;
        pdf.setFont('helvetica', 'normal');
        pdf.text(`   Data: ${ag.dataAgendamento?.toDate?.()?.toLocaleDateString('pt-BR') || 'Não agendado'}`, margin, yPos);
        yPos += lineHeight;
        pdf.text(`   Técnicos: ${ag.tecnicosNomes?.join(', ') || '-'}`, margin, yPos);
        yPos += lineHeight;
        pdf.text(`   Status: ${ag.status}`, margin, yPos);
        yPos += lineHeight;

        if (ag.tipoFinalizacao) {
          pdf.text(`   Finalização: ${ag.tipoFinalizacao === 'sem_pendencia' ? 'Sem pendência' :
            ag.tipoFinalizacao === 'pendencia_tecnica' ? 'Com pendência técnica' :
        ...
        <Paper elevation={0} sx={{ p: 1.5, bgcolor: depth > 0 ? '#f9f9f9' : '#fff', border: '1px solid #eee', borderRadius: 1 }}>
          ...
          {pergunta.tipoResposta === 'assinatura' && assinatura ? (
            ...
            <Typography variant="body2" color="text.primary" sx={{ bgcolor: '#f5f5f5', p: 0.75, borderRadius: 1 }}>
              {formatarResposta(pergunta.id, resposta, pergunta.id) || <em style={{ color: '#999' }}>N�o respondido</em>}
            </Typography>
          )}
        </Paper>
  ...
  const handleSaveAgendamento = async () => {
    ...
    try {
      ...
      console.log('[Email] Usuários disponíves:', usuarios.map(u => ({ id: u.id, nome: u.nome, email: u.email })));
                ...
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                  ...
                  <div style="background-color: #f5f5f5; padding: 20px; border-radius: 0 0 8px 8px;">
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                      ...
                      <tr>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;"><strong>T�cnicos:</strong></td>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;">${tecnicosNomes.join(', ')}</td>
                      </tr>
                      ${agendamentoForm.observacao ? `
                      <tr>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;"><strong>Observa��o:</strong></td>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;">${agendamentoForm.observacao}</td>
                      </tr>
                      ` : ''}
                      ${selectedChamado.descricao ? `
                      <tr>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;"><strong>Descri��o:</strong></td>
                    ...
                    <div style="margin-top: 20px; text-align: center;">
                      <a href="${linkPublico}" 
                        ...
                        Acessar Formul�rio de Atendimento
                      </a>
                    </div>
                    <p style="margin-top: 20px; color: #666; font-size: 12px; text-align: center;">
  ...
  return (
    <Box>
        ...
        <>
          <Box className="mb-4">
            <TextField
              fullWidth
              size="small"
              label="Buscar por Número OS ou Cliente"
                                          ...
                                          <AccordionDetails sx={{ pt: 0, pb: 1.5 }}>
                                            ...
                                            {ag.tipoFinalizacao === 'pendencia_tecnica' && ag.pendenciaTecnica && (
                                              <Alert severity="warning" sx={{ mt: 1 }} icon={<Build fontSize="small" />}>
                                                <Typography variant="caption" fontWeight="bold">Pend�ncia T�cnica:</Typography>
      ...
      {/* Dialog de edi��o (seu existente) - mantenha o seu aqui */}
                ...
                <Stack spacing={0.5}>
                  ...
                  <Typography variant="body2">
                    ...
                    {selectedAgendamentoRespostas.tipoFinalizacao === 'sem_pendencia' ? 'Sem pendência' :
                      selectedAgendamentoRespostas.tipoFinalizacao === 'pendencia_tecnica' ? 'Com pendência ténica' :
