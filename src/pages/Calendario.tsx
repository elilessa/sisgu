...
import { StatusChip } from '../components/TableActions';
import {
  collection,
  onSnapshot,
  orderBy,
...
function MonthView({
  ...
  return (
    <Box sx={{ border: '1px solid rgba(0,0,0,0.12)', borderBottom: 0, borderRight: 0 }}>
      ...
      >
        {weekDaysLabels.map((w, index) => (
          ...
          >
            <Typography sx={{ fontWeight: 900, color: 'text.secondary', fontSize: 12, textTransform: 'uppercase' }}>
              {w}
            </Typography>
          </Box>
        ))}
      ...
      <Box
        sx={{
          display: 'grid',
          gridTemplateColumns: 'repeat(7, 1fr)',
          // Sem gap para as células ficarem coladas
        }}
      >
        {monthDays.map((day) => {
          ...
          return (
            <Paper
              key={day.toISOString()}
              elevation={0}
              square // Garante que não tenha arredondamento
              sx={{
                // Bordas apenas na direita e embaixo de cada célula
                borderRight: '1px solid rgba(0,0,0,0.12)',
                borderBottom: '1px solid rgba(0,0,0,0.12)',
                p: 1,
                minHeight: isDownSm ? 96 : 132,
                bgcolor: isToday ? 'rgba(25,118,210,0.04)' : '#fff',
                opacity: isCurrentMonth ? 1 : 0.4,
                overflow: 'hidden',
                borderRadius: 0, // Força zero arredondamento
                '&:hover': {
                  bgcolor: isToday ? 'rgba(25,118,210,0.08)' : '#fafafa',
                }
              }}
            >
              ...
              <Box sx={{ display: 'grid', gap: 0.5 }}>
                ...
                {dayEvents.length > (isDownSm ? 2 : 3) && (
                  <Typography sx={{ color: 'text.secondary', fontWeight: 900, fontSize: 11, mt: 0.25, textAlign: 'center' }}>
                    +{dayEvents.length - (isDownSm ? 2 : 3)} mais
                  </Typography>
                )}

                {!dayEvents.length && (
                  <Typography sx={{ color: 'text.secondary', fontWeight: 700, fontSize: 12, opacity: 0 }}>
                    —
                  </Typography>
                )}
              </Box>
