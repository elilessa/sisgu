...
export default function Questionarios() {
  ...
  const handleDelete = async (id: string) => {
    if (!userData?.empresaId) return;
    try {
      await deleteDoc(doc(db, 'EMPRESAS', userData.empresaId, 'questionarios', id));
  ...
  const removePergunta = (path: number[]) => {
    setPerguntas((prev) => removePerguntaAtPath(prev, path));
  };
        ...
        {list.map((p, idx) => {
          ...
          return (
            <Accordion
              ...
              onChange={(_, isExp) => setExpandedKey(isExp ? key : false)}
              sx={{
                borderRadius: 2.5,
                overflow: 'hidden',
              ...
              >
                ...
                <Stack direction="row" spacing={1} sx={{ ml: 'auto' }} onClick={(e) => e.stopPropagation()}>
                  ...
                  <Tooltip title="Excluir pergunta">
                    <IconButton size="small" onClick={() => removePergunta(path)} sx={{ color: '#f44336' }}>
              ...
              >
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                  <Tooltip title="Editar">
                    <IconButton onClick={() => openEdit(q)} sx={{ color: '#1976d2' }}>
                      <Edit />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Excluir">
                    <IconButton onClick={() => handleDelete(q.id)} sx={{ color: '#f44336' }}>
                      <Delete />
                    </IconButton>
                  </Tooltip>
                </Stack>
