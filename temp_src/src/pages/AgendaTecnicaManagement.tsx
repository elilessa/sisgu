...
import { Edit, Archive, Add, Visibility, Warning, ExpandMore, Schedule, AttachMoney, Receipt, Unarchive, History, AssignmentReturn, Print } from '@mui/icons-material';
import {
  collection,
  getDocs,
  doc,
...
const STATUS_OPTIONS = [
  ...
  { value: 'pendencia_financeira', label: 'Pendência Financeira', color: 'error' },
...
export function AgendaTecnicaManagement() {
                  ...
                  <Box className="flex items-center gap-2 w-full">
                    ...
                    <Chip
                      ...
                      color={
                        ...
                        chamado.status === 'em_andamento' ? 'info' :
                        chamado.status === 'concluido' ? 'success' :
                        chamado.status === 'cancelado' ? 'error' :
                        chamado.status === 'pendencia_tecnica' ? 'warning' :
                        chamado.status === 'pendencia_financeira' ? 'warning' : 'default'
                      }
                      size="small"
                    />
                  </Box>
              ...
              >
                ...
                >
                  ...
                  <Chip
                    ...
                    color={chamado.tipoChamado === 'tecnico' ? 'primary' : 'secondary'}
                    size="small"
                  />
                  <Chip
                          ...
                          {chamado.status === 'aguardando_devolucao' && chamado.pendenciaRetornoTecnico && (
                            ...
                            >
                              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                                ...
                                <Chip label="Ação Necessária" color="error" size="small" />
                              </Box>
                          ...
                          {chamado.status === 'orcamento_aprovado' && chamado.pendenciaRetornoTecnico && (
                            ...
                            >
                              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                                ...
                                <Chip label="Agendar Execução" color="success" size="small" />
                              </Box>
                            ...
                            chamado.agendamentos.map((ag, idx) => (
                              <Box key={ag.id} sx={{ mb: 1, p: 1, bgcolor: '#f9f9f9', borderRadius: 1 }}>
                                ...
                                <Chip 
                                  label={ag.status} 
                                  size="small" 
                                  color={ag.status === 'concluido' ? 'success' : ag.status === 'agendado' ? 'info' : 'warning'}
                                  sx={{ mt: 0.5 }}
                                />
                              </Box>
                            ))
        ...
        <DialogContent sx={{ px: 3, py: 2.5 }}>
          <Grid container spacing={2.25}>
            ...
            <Grid size={{ xs: 12 }}>
              ...
              >
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, md: isEditing ? 6 : 12 }}>
                    <TextField
                      fullWidth
                      type="datetime-local"
                      label="Data e Hora *"
                  ...
                  {/* Campo OS (somente leitura) - agora opcional aqui dentro se quiser manter além do badge */}
                  {isEditing && (
                    <Grid size={{ xs: 12, md: 6 }}>
                      <TextField
                        fullWidth
                        label="Nº da O.S."
                        value={selectedChamado?.numeroOS || ''}
                        InputProps={{ readOnly: true }}
                        sx={{
                          '& .MuiInputBase-root': { minHeight: 56, bgcolor: '#f5f5f5' },
                        }}
                      />
                    </Grid>
                  )}
                  ...
                  {/* Tipo / Status em layout mais responsivo */}
            ...
            <Grid size={{ xs: 12 }}>
              ...
              <TextField
                ...
                sx={{
                  '& .MuiInputBase-root': { minHeight: 140 },
                }}
              />
            </Grid>
          </Grid>
